...
(#77, #94, #88) ran on `R3CY205ZMND` (SM-S938N). The NPU **"now"** is #88's
variant B (2026-09-22, `3f9fa38d`) = the code of `htp_moe` @ `ad714de7`
(PR #103 merged; M=1 GEMV still off by default); the CPU "now" and the
first-lever reading (variant C, LEDGER ⑯) stay from #94 sitting 2
(`dad0f476`), which #88 did not re-run.

## Goals

| goal | now | target | ceiling | status |
|---|---|---|---|---|
| decode tok/s, NPU, gen 64 / 512 / 1024 | **24.50 / 23.80 / 23.52** (#88 B, means of two runs, `R3CY205ZMND`, PR #103 @ `f3e99176` = the code of `htp_moe` @ `ad714de7`, GEMV off); its own same-sitting A (`08afbb10`, = #94 s2's A sources) read 18.72 / 16.83 / 16.46, so **+30.9 / +41.4 / +42.9 %** (LEDGER ⑦). Earlier: #94 sitting 2 A 17.72 / 17.03 / 17.30 with variant C (`NNTR_MOE_HTP_M1_GEMV=1`) 18.83 / 18.33 / 17.59; #77 19.7 / 21.3 · 18.4 / 18.2 · 19.0 / 17.0 (same unit); 20.8 (PR, gen 512, prompt 444) | **≥ 50** at each length | 48–52 (730 MB/token ÷ 34–38 GB/s) | distance **2.1× at G=512** (50 / 23.80; was 3.0× on #88's own A). Wall 3 is closed (LEDGER ⑦); the MoE *call* is now 94 % DSP (1383 of 1471 µs). **Per-token budget at G=512, 42.0 ms (cycle 7 estimate: TPS minus 22 × the level-2 M==1 row, LEDGER §2):** MoE dsp 22 × 1.383 = **30.4 ms (72 %)**, its DDR floor 484 MB ÷ 38 GB/s = 12.7; transport 22 × 0.088 = **1.9 ms (5 %)**; everything outside the MoE call (FC, conv, attention, norms, lm_head) **≈ 9.6 ms (23 %)** (8.4 at G=64), its floor 246 MB ÷ 38 GB/s ≈ 6.5. The biggest lever is the MoE dsp column (walls 1–2: #101, #105, #99/#100); at the MoE floor the token is ≈ 24 ms (≈ 41 tok/s), so ⑨ (#85) is needed for the last 20 % |
