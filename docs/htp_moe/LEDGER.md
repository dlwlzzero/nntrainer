...
| ⑦ | Transport (wall 3) **resolved by #77 B to prebound handles + per-call buffer/marshalling cleanup on plain FastRPC — #88**. **PR #103 (`htp/88-moe-call-marshalling`): size-class staging + 5 ms poll cherry-picked, `staging:` inventory line added; awaiting the handoff `88-moe-call-marshalling.md` (A = `08afbb10`, B = PR, C = B + `NNTR_HTP_POLL_US=100`)**; #83 narrowed to the document that says what "prebound" concretely means (inventory of per-call bytes, bind/run IDL pair, persistent ION staging; upstream `fb0f02b9` size-class staging is the author's step in the same area). dspqueue / resident worker: one paragraph, revisited only with ⑨ | 0.53 → ≤ 0.1 ms per call (−10 ms/token) | #83 → #88 |
...
| ㉒ | **GEMV weight feed (the next lever inside ⑯; not yet an issue).** C's `mm` 783 → 975 µs is a direct arena read at ≈ 22.6 GB/s effective; a VTCM prefetch of the four experts' tiles ahead of the `vrmpy` loop would bring `mm` toward the HMX loop's 783 or below — if the feed does not inherit row h's slow list. Filed once #100 names h's cause; gate then = `mm` ≤ 600 µs at M==1 with text identical | MoE dsp 1044 → ≈ 700 µs/call (−7 ms/token) | #100 |
