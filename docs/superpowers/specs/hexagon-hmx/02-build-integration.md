...
- `~/hexkl_addon/lib/6.3.0.0/hexagon_toolv88_v75|v79/libhexkl_micro.a`를 `libnntr_skel.so`에 정적 링크. 헤더는 `~/hexkl_addon/include/hexkl_micro.h`
- `tools/hexagon/build_skel.sh` 및 meson에 `HEXKL_ADDON_ROOT` 경로 변수 추가. SDK 버전(`HEXAGON_SDK_ROOT`)에서 `lib/<sdk_version>/` 자동 선택 (hexkl_addon 자체 스크립트와 같은 규칙)
...
대신 기존 확보 코드에 HMX 파라미터 한 줄만 추가한다:
...
- v75/v79 크로스 빌드에서 `libnntr_skel.so`가 hexkl 심볼 포함하여 링크 성공
