    ...
    if (htp.htp_ops_mat_mul_af32_wf16_of32 && htp.alloc_shared_mem_buf &&
      ...
      if (handle != 0) {
        ...
        if (err == 0 && htp_offset_fits_int(out_offset)) {
          ...
          err = htp.htp_ops_mat_mul_af32_wf16_of32(handle, io_fd, out_offset,
                                                   io_fd, act_offset, io_fd,
                                                   wt_offset, M, K, N);
          if (err == 0) {
          ...
          if (it == g_htp_weight_cache.end()) {
            ...
            if (htp.alloc_shared_mem_buf(&wbuf, &new_fd, wt_size) == 0 &&
              ...
              it = g_htp_weight_cache
                     .emplace(wkey, HtpCachedWeight{new_fd, wbuf, wt_size})
                     .first;
              wt_fd = new_fd;
              wt_ok = true;
            }
...
Tensor &FloatTensor::dotQnK(Tensor const &input, Tensor &output, bool trans,
      ...
      if (htp.htp_ops_mat_mul_af32_pwqk0_of32 && htp.alloc_shared_mem_buf &&
        ...
        if (handle != 0 && K % 256 == 0 && N % 32 == 0) {
          ...
          Q4_0Utils::repackToX4x2_Q4_0x4(
            reinterpret_cast<const block_q4_0x4 *>(mdata), weight_x4x2.data(),
            N, K, &actual_stride);
          ...
          if (err == 0) {
            ...
            err = htp.htp_ops_mat_mul_af32_pwqk0_of32(handle, out_fd, 0, act_fd,
                                                      0, wt_fd, 0, M, K, N, 2);
            if (err == 0) {
  ...
  case Tdatatype::Q4_0_X4X2: {
    ...
    K = getDim().width();
    N = input.getDim().width();
    const size_t row_stride = (size_t)(K / 2) + (size_t)(K / 256) * 16;
#if defined(ENABLE_HTP) && ENABLE_HTP == 1
    {
      auto &htp = nntrainer::htp::HtpInterface::instance();
      if (htp.htp_ops_mat_mul_af32_pwqk0_of32 && htp.alloc_shared_mem_buf &&
        ...
        if (handle != 0 && K % 256 == 0 && N % 32 == 0) {
          constexpr size_t kAlign = 128;
          const size_t act_size = (size_t)M * K * sizeof(float);
          const size_t wt_size = (size_t)N * row_stride;
          const size_t out_size = (size_t)M * N * sizeof(float);

          // ---- 1. Weight: x4x2 bytes are already in the loaded tensor. Map
          //      them into RPC shared memory once per distinct weight pointer
          //      and reuse on every subsequent call (no per-call repack,
          //      alloc, memcpy or free). Mirrors the FP16 cached path.
          int wt_fd = -1;
          bool wt_ok = false;
          {
            const void *wkey = static_cast<const void *>(mdata);
          ...
          if (wt_ok) {
            // ---- 2. Scratch: per-thread act+out staging, grown lazily.
            const size_t act_off = 0;
            const size_t out_off = htp_align_up(act_size, kAlign);
            const size_t need = htp_align_up(out_off + out_size, kAlign);
            ...
            if (scratch_ok && htp_offset_fits_int(out_off)) {
              ...
              // Kernel failure: fall through to CPU path; cache stays alive.
            }
          }
        }
      }
...
#endif
    // ---- CPU fallback: x4x2 -> plain q4_0 -> repack(x4/x8) -> gemm_q4_0_fp32.
    // gemm_q4_0_fp32 consumes the interleaved block_q4_0x4 layout on ARM, so
    // plain blocks must be repacked first.
    {
      const size_t n_blocks = (size_t)N * (K / QK4_0);
      std::vector<block_q4_0> plain(n_blocks);
      Q4_0Utils::unpackX4x2_to_Q4_0(mdata, plain.data(), N, K, row_stride);
      std::vector<uint8_t> repacked(n_blocks * sizeof(block_q4_0));
      // Qualify with nntrainer:: : cpu_backend.h declares a global-namespace
      // ::repack_q4_0 while the backend header declares nntrainer::repack_q4_0,
      // so an unqualified call from inside namespace nntrainer is ambiguous.
      nntrainer::repack_q4_0(repacked.data(), plain.data(), repacked.size(), N,
                             K, ml::train::ISA::DEFAULT);
      o->gemm_q4_0_fp32(M, N, K, data, K, (void *)repacked.data(), N, rdata, N);
    }
    break;
  }
