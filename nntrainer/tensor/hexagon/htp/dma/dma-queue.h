...
/**
 * @file	dma-queue.h
 * @brief	Imported from llama.cpp ggml-hexagon (MIT). Local changes noted
 * inline.
 * @author	The ggml authors (upstream); imported by dlwlzzero
 * <dlwlzzero@gmail.com>
...
static inline void htp_trace_event_start(struct htp_thread_trace *tr,
                                         uint16_t id, uint16_t info) {
  (void)tr;
  (void)id;
  (void)info;
}
static inline void htp_trace_event_stop(struct htp_thread_trace *tr,
                                        uint16_t id, uint16_t info) {
  (void)tr;
  (void)id;
  (void)info;
...
// Define the HW descriptor structs here since the ones in HexSDK are a bit out
// of date
...
dma_queue_t dma_queue_init(void *ptr, size_t capacity, uintptr_t vtcm_base,
                           size_t vtcm_size, struct htp_thread_trace *trace);
...
// TODO: technically we don't need these and could use Q6_dmstart/wait/etc
// instead but those do not seem to always compiler properly.
...
static inline bool dma_is_vtcm(const dma_queue *q, const void *ptr) {
  return (uintptr_t)ptr >= q->ring->vtcm_base &&
         (uintptr_t)ptr < q->ring->vtcm_end;
}

static inline bool dma_queue_push_single_1d(dma_queue *q, dma_ptr dptr,
                                            size_t size) {
  dma_ring *r = q->ring;
...
static inline bool dma_queue_push_single_2d(dma_queue *q, dma_ptr dptr,
                                            size_t dst_stride,
                                            size_t src_stride, size_t row_size,
                                            size_t nrows) {
  dma_ring *r = q->ring;
  if (((r->push_idx + 1) & r->idx_mask) == r->pop_idx) {
    return false;
...
// Overflow-safe DMA push: all 2d descriptor fields (row_size, nrows,
// src_stride, dst_stride) are 16-bit, max 65535. This version transparently
// handles values that exceed the 16-bit limit and submits chained DMA
// transtions.

#define DMA_MAX_FIELD_VAL 65535u

static inline bool dma_queue_push(dma_queue *q, dma_ptr dptr, size_t dst_stride,
                                  size_t src_stride, size_t row_size,
                                  size_t nrows) {
  // Fast path: everything fits in 16 bits
  if (nrows == 0 || __builtin_expect(row_size <= DMA_MAX_FIELD_VAL &&
                                       nrows <= DMA_MAX_FIELD_VAL &&
                                       src_stride <= DMA_MAX_FIELD_VAL &&
                                       dst_stride <= DMA_MAX_FIELD_VAL,
                                     1)) {
    return dma_queue_push_single_2d(q, dptr, dst_stride, src_stride, row_size,
                                    nrows);
  }
...
static inline bool dma_queue_push_ddr_to_vtcm(dma_queue *q, dma_ptr dptr,
                                              size_t dst_row_size,
                                              size_t src_row_size,
                                              size_t nrows) {
  return dma_queue_push(q, dptr, dst_row_size, src_row_size, src_row_size,
                        nrows);
}

static inline bool dma_queue_push_vtcm_to_ddr(dma_queue *q, dma_ptr dptr,
                                              size_t dst_row_size,
                                              size_t src_row_size,
                                              size_t nrows) {
  return dma_queue_push(q, dptr, dst_row_size, src_row_size, dst_row_size,
                        nrows);
}
...
static inline void dma_cache_init(dma_cache *c, uint8_t *base,
                                  uint32_t line_size, uint32_t capacity) {
...
static inline bool dma_cache_push(dma_queue *q, dma_cache *c,
                                  const uint8_t *src, uint32_t dst_stride,
  ...
  if (!dst) {
    ...
    return dma_queue_push(q, dma_make_ptr(dst, src), dst_stride, src_stride,
                          row_size, nrows);
  }
