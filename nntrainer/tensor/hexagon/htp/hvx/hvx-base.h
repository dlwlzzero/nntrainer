...
static inline void hvx_vec_store_u(void *restrict dst, uint32_t n,
                                   HVX_Vector v) {
  // Rotate as needed.
...
static inline void hvx_vec_store_a(void *restrict dst, uint32_t n,
                                   HVX_Vector v) {
  assert((unsigned long)dst % 128 == 0);
  HVX_VectorPred m =
    Q6_Q_or_QQn(Q6_Q_vsetq_R((unsigned long)dst), Q6_Q_vsetq2_R(n));
...
static inline HVX_Vector hvx_vec_repl4(HVX_Vector v) {
  ...
  static const uint8_t __attribute__((aligned(128))) repl[128] = {
    0x00, 0x00, 0x00, 0x00, 0x04, 0x04, 0x04, 0x04, 0x08, 0x08, 0x08, 0x08,
    0x04, 0x04, 0x04, 0x04, 0x10, 0x10, 0x10, 0x10, 0x04, 0x04, 0x04, 0x04,
    0x08, 0x08, 0x08, 0x08, 0x04, 0x04, 0x04, 0x04, 0x20, 0x20, 0x20, 0x20,
    0x04, 0x04, 0x04, 0x04, 0x08, 0x08, 0x08, 0x08, 0x04, 0x04, 0x04, 0x04,
    0x10, 0x10, 0x10, 0x10, 0x04, 0x04, 0x04, 0x04, 0x08, 0x08, 0x08, 0x08,
    0x04, 0x04, 0x04, 0x04, 0x40, 0x40, 0x40, 0x40, 0x04, 0x04, 0x04, 0x04,
    0x08, 0x08, 0x08, 0x08, 0x04, 0x04, 0x04, 0x04, 0x10, 0x10, 0x10, 0x10,
    0x04, 0x04, 0x04, 0x04, 0x08, 0x08, 0x08, 0x08, 0x04, 0x04, 0x04, 0x04,
    0x20, 0x20, 0x20, 0x20, 0x04, 0x04, 0x04, 0x04, 0x08, 0x08, 0x08, 0x08,
    0x04, 0x04, 0x04, 0x04, 0x10, 0x10, 0x10, 0x10, 0x04, 0x04, 0x04, 0x04,
...
static inline HVX_VectorPred hvx_vec_is_nan_f16(HVX_Vector v) {
  ...
  // get pred of which are NaN, i.e., exponent bits all 1s and fraction bits non
  // 0s
  HVX_VectorPred p_exp = Q6_Q_vcmp_eq_VhVh(Q6_V_vand_VV(v, vnan_exp), vnan_exp);
  HVX_VectorPred p_frac =
    Q6_Q_not_Q(Q6_Q_vcmp_eq_VhVh(Q6_V_vand_VV(v, vnan_frac), vnan_exp));
  return Q6_Q_and_QQ(p_exp, p_frac);
}

static inline HVX_Vector hvx_vec_f32_to_f16_shuff(HVX_Vector v0,
                                                  HVX_Vector v1) {
#if __HVX_ARCH__ >= 81
...
static inline HVX_VectorPair hvx_vec_f16_to_f32_shuff(HVX_Vector v) {
  ...
  return Q6_W_vcombine_VV(Q6_Vsf_equals_Vqf32(Q6_V_hi_W(p)),
                          Q6_Vsf_equals_Vqf32(Q6_V_lo_W(p)));
}
static inline HVX_VectorPair hvx_vec_f16_to_f32(HVX_Vector v) {
  const HVX_Vector one = hvx_vec_splat_f16(1.0);
...
static inline HVX_Vector hvx_vec_i16_from_hf_rnd_sat(HVX_Vector vin) {
  ...
  HVX_Vector vsf_0 =
    Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_Vqf32Vsf(Q6_V_lo_W(v32), v192K));
  HVX_Vector vsf_1 =
    Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_Vqf32Vsf(Q6_V_hi_W(v32), v192K));

  // for in-range cases, result is {163858... 229360} so the exponent is always
  // 144. if we extract bits 21..0 as a signed quantity, and round 6 bits off,
  // that will be the answer. Start by <<10 to get the final 'sign' bit in
  // bit 15...
...
hvx_vec_mpyacc_f32_f16(HVX_VectorPair acc, HVX_Vector x, HVX_Vector y) {
  ...
  HVX_Vector a0 =
    Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_Vqf32Vsf(Q6_V_lo_W(m), Q6_V_lo_W(acc)));
  HVX_Vector a1 =
    Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_Vqf32Vsf(Q6_V_hi_W(m), Q6_V_hi_W(acc)));
