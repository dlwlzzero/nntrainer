...
#include "htp/hmx_mgr.h"
#include "htp/mmap_mgr.h"
...
//                             int32 query_offset, int32 key_fd, int32 key_offset, int32 k_seq_len, 
//                             int32 n_kv_heads, int32 head_dim, int32 gqa_size, int32 seq_len) {
...
//   float        *output  = (float *) (p0 + output_offset);
//   const __fp16 *query   = (const __fp16 *) (p1 + query_offset);
//   const __fp16 *key     = (const __fp16 *) (p2 + key_offset);

//   int n_q_heads = n_kv_heads * gqa_size;
//   size_t output_size = k_seq_len * (seq_len * n_q_heads) * sizeof(float);
//   size_t query_size  = seq_len * (head_dim * n_q_heads) * sizeof(__fp16); 
//   size_t key_size    = k_seq_len * (n_kv_heads * head_dim) * sizeof(__fp16);
...
//   FILE *fp;
//   fp = fopen("/data/local/tmp/htp_lib/log.txt","w");

//   fprintf(fp, "Print Origin Data in RPCMEM\n");
//   for(int i = 0; i < k_seq_len; ++i){
//     for(int j = 0; j < (n_kv_heads * head_dim); ++j){
//       uint16_t *ptr = (uint16_t *)&key[i*(n_kv_heads * head_dim) + j];
//       fprintf(fp, "%04x,", *ptr);
//     }
//     fprintf(fp, "\n");
//   }

//   // err = hmx_attention(output, query, key, seq_len, k_seq_len, n_kv_heads, head_dim, gqa_size);
