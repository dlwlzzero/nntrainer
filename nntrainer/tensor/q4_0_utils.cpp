...
void Q4_0Utils::repackToX4x2_Q4_0(const block_q4_0 *src_q4_0, uint8_t *dst_x4x2,
  ...
  const size_t scales_per_row =
    n_superblocks_per_row * 16; // 16 bytes per scale block
  ...
  *out_row_stride = row_stride;
  ...
  for (size_t row = 0; row < N; ++row) {
    ...
    // Pack quants: interleave 4 groups into x4x2 sub-blocks (128 bytes per 256
    // elements)
...
void Q4_0Utils::unpackX4x2_to_Q4_0(const uint8_t *src_x4x2,
                                   block_q4_0 *dst_q4_0, size_t N, size_t K,
                                   size_t row_stride) {
  assert(K % 256 == 0);
  const size_t n_groups_per_row = K / QK4_0;
  const size_t quants_per_row = K / 2;
  const size_t n_superblocks_per_row = K / 256;

  for (size_t row = 0; row < N; ++row) {
    const uint8_t *src_row = src_x4x2 + row * row_stride;
    const uint8_t *src_quants = src_row;
    const uint8_t *src_scales = src_row + quants_per_row;

    block_q4_0 *dst_row = dst_q4_0 + row * n_groups_per_row;

    for (size_t sb = 0; sb < n_superblocks_per_row; ++sb) {
      const uint8_t *sb_quants = src_quants + sb * 128;
      const uint16_t *sb_scales =
        reinterpret_cast<const uint16_t *>(src_scales + sb * 16);

      for (int g = 0; g < 8; ++g) {
        block_q4_0 *group = &dst_row[sb * 8 + g];
        int sub_blk = g / 4;
        int sub_idx = g % 4;
        std::memcpy(group->qs, sb_quants + sub_blk * 64 + sub_idx * 16,
                    QK4_0 / 2);
        group->d = sb_scales[g];
      }
    }
...
void quantizeAndRepackQ4_0X4x2(const float *weight_fp32, uint8_t *dst_x4x2,
  ...
  std::vector<char> tmp(n_blocks * sizeof(block_q4_0));
  quantize_q4_0(weight_fp32, tmp.data(), N, K, nullptr);
  size_t row_stride = 0;
  Q4_0Utils::repackToX4x2_Q4_0(reinterpret_cast<const block_q4_0 *>(tmp.data()),
                               dst_x4x2, N, K, &row_stride);
}
