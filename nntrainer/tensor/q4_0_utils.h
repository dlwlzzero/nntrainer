...
public:
  ...
  /**
   * @brief Inverse of repackToX4x2_Q4_0: convert x4x2 row-strided bytes back
   * to plain block_q4_0 blocks (N rows x K/32 blocks). Used by the CPU
   * fallback when HTP is unavailable. Exact numerical inverse of the forward
   * repack (round-trip is byte-identical).
   * @param[in] src_x4x2 source data in x4x2 row-strided format
   * @param[out] dst_q4_0 output block_q4_0 blocks (N * K/32)
   * @param[in] N number of output rows
   * @param[in] K number of input columns (must be divisible by 256)
   * @param[in] row_stride bytes per row in the source (K/2 + (K/256)*16)
   */
  static void unpackX4x2_to_Q4_0(const uint8_t *src_x4x2, block_q4_0 *dst_q4_0,
                                 size_t N, size_t K, size_t row_stride);
   ...
   * @param[out] out_row_stride bytes per row in the output
   */
  static void repackToX4x2_Q4_0x4(const block_q4_0x4 *src_x4, uint8_t *dst_x4x2,
                                  size_t N, size_t K, size_t *out_row_stride);
   ...
   * @param[out] out_row_stride bytes per row in the output
   */
  static void repackToX4x2_Q4_0x8(const block_q4_0x8 *src_x8, uint8_t *dst_x4x2,
                                  size_t N, size_t K, size_t *out_row_stride);
   ...
   */
  static void repackToX4x2_Q8_0(const int8_t *src_q8_0, const uint16_t *scales,
