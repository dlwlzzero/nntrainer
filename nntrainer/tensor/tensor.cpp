...
Tensor::Tensor(std::string name_, Tformat fm, Tdatatype d_type) {
  ...
  } else if (d_type == Tdatatype::Q4_0_X4X2) {
    itensor_ = std::make_unique<Q4_0_X4X2_Tensor>(name_, fm);
...
Tensor::Tensor(const TensorDim &d, bool alloc_now, Initializer init,
  ...
  } else if (d.getDataType() == Tdatatype::Q4_0_X4X2) {
    itensor_ = std::make_unique<Q4_0_X4X2_Tensor>(d, alloc_now, init, name);
...
Tensor::Tensor(const TensorDim &d, const void *buf, QScheme qscheme) {
  ...
  if (d.getDataType() == Tdatatype::FP32) {
    ...
    itensor_ = std::make_unique<Q4_0_X4X2_Tensor>(d, buf);
  } else if (d.getDataType() == Tdatatype::UINT4) {
    if (qscheme != QScheme::Q4_Kx8)
...
Tensor::Tensor(const Tensor &rhs) {
  ...
  } else if (rhs.getDataType() == Tdatatype::Q4_0_X4X2) {
    itensor_ = std::make_unique<Q4_0_X4X2_Tensor>(*rhs.itensor_);
...
bool Tensor::operator==(const Tensor &rhs) const {
  ...
  if (*itensor_.get() == *rhs.itensor_.get()) {
    ...
    } else if (getDataType() == Tdatatype::Q4_0_X4X2) {
      return itensorCompare<Q4_0_X4X2_Tensor>(itensor_.get(),
                                              rhs.itensor_.get());
